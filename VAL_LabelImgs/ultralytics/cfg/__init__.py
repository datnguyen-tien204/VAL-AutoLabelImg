...
from VAL_LabelImgs.ultralytics.cfg.create_default_yaml import create_default_yaml
import yaml
...
def get_cfg(cfg: Union[str, Path, Dict, SimpleNamespace] = DEFAULT_CFG_DICT, overrides: Dict = None):
    ...
    Examples:
        >>> from VAL_LabelImgs.ultralytics.cfg import get_cfg
...
def handle_yolo_hub(args: List[str]) -> None:
    ...
    """
    from VAL_LabelImgs.ultralytics import hub
...
def entrypoint(debug=""):
    ...
    if "rtdetr" in stem:  # guess architecture
        from VAL_LabelImgs.ultralytics import RTDETR

        model = RTDETR(model)  # no task argument
    elif "fastsam" in stem:
        from VAL_LabelImgs.ultralytics import FastSAM

        model = FastSAM(model)
    elif "sam_" in stem or "sam2_" in stem:
        from VAL_LabelImgs.ultralytics import SAM
...
def copy_default_cfg():
    ...
    Examples:
        >>> copy_default_cfg()
        # Output: default_imgs.yaml copied to /path/to/current/directory/default_copy.yaml
